    ...
    private getLeadDashboardHtml(project: Project): string {
        ...
        <body>
            ...
            <div class="section">
                ...
                ${tasks.filter(t => t.assignee).length === 0 ? '<p>No assigned tasks.</p>' :
                `<ul>${tasks.filter(t => t.assignee).map(t => `
                    <li style="margin-bottom: 8px; border-left: 3px solid #64748b; padding-left: 8px;">
                        <div><strong>${t.name}</strong></div>
                        <div style="font-size: 0.9em; opacity: 0.8;">Assigned to: ${t.assignee}</div>
                    </li>`).join('')}</ul>`
            }
            </div>
